...
var str = "这一生 ， 需要历经多少场巨浪风波 ，才能心尖造王国 ，富饶而安逸 ，盛大而静默 ，置身边城荒漠 ，也有那无名花一朵 。";
var oArr = str.split('');
...
function textmove(){
     ...
     timer = setInterval(function(){
        if(q<oArr.length){
            oSett.innerHTML += oArr[q];
            oGett.value = oSett.value;
        }else{
            clearInterval(timer);
        }
        q++;
...
$(function(){
    ...
    $(document).on('scroll',{name:'jqI'},function(z){
        var jqMax = dImg.offsetTop - H*3/5;
        var T = getPos().scrollT;
            if(jqMax<T){
                move(dImg,{marginLeft:0,opacity:100});
                $( this ).off( z );
            }
     });
...
function Qmove(obj){
    ...
    timer = setInterval(function(){
        ...
        if(t>=bmaxH+10){
            clearInterval(timer);
            oBanner.style.borderColor = HT1.style.color = '#f00';
